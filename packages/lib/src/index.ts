...
import { devNuxtRemotePlugin } from './dev/nuxt-remote-development'
import { prodNuxtRemotePlugin } from './prod/nuxt-remote-production'
import { prodNuxtExposePlugin } from './prod/nuxt-expose-production'
import { prodNuxtSharedPlugin } from './prod/nuxt-shared-production'
...
export function nuxtFederationVitePlugin(
  ...
  function registerPlugins(mode: string, command: string) {
    if (mode === 'production' || command === 'build') {
      pluginList = [
        prodNuxtSharedPlugin(options),
        prodNuxtExposePlugin(options),
        prodNuxtRemotePlugin(options)
      ]
    } else if (mode === 'development' || command === 'serve') {
      pluginList = [
        devSharedPlugin(options),
        devExposePlugin(options),
        devNuxtRemotePlugin(options)
      ]
    } else {
      pluginList = []
    }
  ...
  return {
    ...
    async resolveId(...args) {
      ...
      const sharedModule = parsedOptions.prodShared.find(
        (item) => item[0] === args[0]
      )
      if (sharedModule) {
        const sharedPath = normalizePath(sharedModule[1].packagePath)
        return sharedPath
      }
      if (args[0] === '\0virtual:__federation_fn_import') {
        return {
